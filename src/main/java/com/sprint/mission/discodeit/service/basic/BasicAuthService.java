...
public class BasicAuthService implements AuthService {
    ...
    public UserDto updateRole(RoleUpdateRequest request) {
        ...
        Role oldRole = foundUser.getRole();
        Role newRole = request.newRole();

        foundUser.updateRole(newRole);
        UUID updatedUserId = foundUser.getId();

        applicationEventPublisher.publishEvent(new RoleUpdatedEvent(updatedUserId, oldRole, newRole));

        jwtRegistry.invalidateJwtInformationByUserId(updatedUserId);
        return userMapper.toDto(foundUser);
    }

    @Override
    @Transactional
    public UserDto updateRoleWithoutEvent(RoleUpdateRequest request) {
        UUID userId = request.userId();
        User foundUser = userRepository.findById(userId)
                .orElseThrow(() -> UserNotFoundException.withId(userId));
        ...
        jwtRegistry.invalidateJwtInformationByUserId(foundUser.getId());
        return userMapper.toDto(foundUser);
    }
