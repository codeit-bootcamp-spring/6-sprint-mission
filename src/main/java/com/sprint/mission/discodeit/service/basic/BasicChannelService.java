...
public class BasicChannelService implements ChannelService {
    ...
    @Transactional(readOnly = true)
    @Override
    @Cacheable(cacheNames = CacheNames.USER_CHANNELS, key = "#userId")
    public List<ChannelDto> findAllByUserId(UUID userId) {
        return loadChannelsByUserId(userId);
    }
    ...
    private List<ChannelDto> loadChannelsByUserId(UUID userId) {
    ...
    private void refreshUserChannelsCache() {
        List<UUID> userIds = userRepository.findAll().stream()
            .map(User::getId)
            .toList();
        for (UUID userId : userIds) {
            cacheUpdater.putUserChannels(userId, loadChannelsByUserId(userId));
        }
    }
}
