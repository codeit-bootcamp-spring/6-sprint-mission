...
public class BasicChannelService implements ChannelService {
    ...
    public ChannelDto create(PrivateChannelCreateRequest request) {
        ...
        log.info("채널 생성 완료: id={}, name={}", channel.getId(), channel.getName());
        channelCacheService.evictForUsers(request.participantIds()); //캐시 처리 추가
        return channelMapper.toDto(channel);
    }
    ...
    public void delete(UUID channelId) {
        ...
        if (foundChannel.getType().equals(ChannelType.PRIVATE)) {
            List<UUID> userIds = readStatusRepository.findAllByChannelIdWithUser(channelId)
                    .stream().map(readStatus -> readStatus.getUser().getId())
                    .toList();
            channelCacheService.evictForUsers(userIds);
        } else {
            channelCacheService.evictAll();
        }
